...
macro match*(val: ADTBase, branches: varargs[untyped]): untyped =
  ...
  for branch in branches:
    if branch.kind in {nnkElse, nnkElifBranch}:
      ...
      let body = branch[^1]
      for branch in branch[0..^2]:
        let injection = body.copyNimTree()
        case branch.kind
        of nnkInfix: # We're doing a named match
          if branch[0].kind != nnkIdent or not branch[0].eqIdent"as":
            error("Invaid operation expected 'as'.", branch[0])

          let (kind, dataName) = getKindAndDataName(adtData, branch[1])
          case branch[^1].kind
          of nnkIdent: # emit a `let`
            injection.insert 0, newLetStmt(branch[^1], newCall("to", val, branch[1]))
            result.add nnkElifBranch.newTree(
              infix(nnkDotExpr.newTree(val, ident"kind"), "==", kind),
              injection)

          of nnkCall, nnkCommand: # Check if it's 'mut', and `val` is mut, emit a template
            if not branch[^1][0].eqIdent"mut":
              error("Can only make a 'mut' call.", branch[^1][0])
            ...
            injection.insert 0:
              genAst(val, byaddr = bindSym"byaddr", name, destType = branch[1]):
          ...
          of nnkTupleConstr: # same as a call check if each a param is a `mut`, if soe emit a template per param
            for i, x in branch[^1]:
              ...
              of nnkCall, nnkCommand:
                ...
                injection.insert 0:
                  genast(val, dataName, name = x[1], index = newLit(i), destType = branch[1], expr = branch[0].repr):
              ...
              of nnkIdent:
                if not x.eqIdent"_":
                  ...
                  injection.insert 0:
                    genast(val, dataName, name = x, index = newLit(i), destType = branch[1], expr = branch[0].repr):
                      when val.dataName isnot tuple:
                        {.error: "attempted to unpack a type that is not a tuple: '" & expr & "'.".}
            ...
            error("Invalid alias statement", branch[^1])
          implemented.incl branch[1].repr

        of nnkIdent, nnkSym: # Just a kind match
          let (kind, _)= getKindAndDataName(adtData, branch)
          result.add nnkElifBranch.newTree(
            infix(nnkDotExpr.newTree(val, ident"kind"), "==", kind),
            injection)
          implemented.incl branch.repr
